...
//----------------------------------------------//
    ...
    bool deCodeAudio(){
        ...
        int frameSize=av_samples_get_buffer_size(NULL,m_codeCtx->ch_layout.nb_channels,m_frame->nb_samples,m_codeCtx->sample_fmt,1);
        uint8_t* frameBuffer=(static_cast<uint8_t*>(av_malloc(frameSize)));
        avcodec_fill_audio_frame(m_frame,m_codeCtx->ch_layout.nb_channels,m_codeCtx->sample_fmt,frameBuffer,frameSize,1);

        m_pkt=av_packet_alloc();
        while(av_read_frame(m_inCtx,m_pkt)==0){
            if(m_pkt->stream_index==audioIndex){
                deCodeAudio_PCM(m_pkt);
            }
            av_packet_unref(m_pkt);
        }
        deCodeAudio_PCM(NULL);
        return true;
    }
    bool deCodeAudio_PCM(AVPacket* pkt){
